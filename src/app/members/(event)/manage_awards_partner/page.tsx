...
import { getEventPartners } from "@/lib/services/eventPartners";
import { EventPartnersManager } from "@/components/dashboard/EventPartnersManager";

export const dynamic = "force-dynamic";
export const metadata = { title: "Manage Awards Partner | Event Management" };
...
function Breadcrumb({ eventId }: { eventId?: number }) {
  return (
    <nav className="mb-6 flex flex-wrap items-center gap-2 text-xs font-semibold text-zinc-400">
      ...
      <ChevronRight className="h-3 w-3 text-zinc-600" />
      <span className="text-brand-pink font-bold">Manage Awards Partner</span>
      {eventId ? <span className="text-zinc-500"> (Event #{eventId})</span> : null}
    </nav>
  );
}

export default async function ManageAwardsPartnerPage({
  searchParams,
}: {
  searchParams: Promise<{ event_id?: string; filter?: string; keyword?: string }>;
}) {
  ...
  const { partners, counts } = await getEventPartners(context, {
    keyword: resolvedParams.keyword,
    filter: resolvedParams.filter,
  });

  const isOrganiser = context.role === "organiser";
  const isFranchise = false; // can be adjusted or checked
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-black uppercase tracking-tight text-white flex items-center gap-2">
                Manage Awards Partner <span className="inline-block h-2 w-2 rounded-full bg-emerald-400 animate-ping" />
              </h1>
              <p className="text-xs font-medium text-zinc-400 mt-1">
                Configure awards partners, registrations, accounts, and communications for Event #{eventId}.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider bg-fuchsia-500/20 text-fuchsia-300 border border-fuchsia-500/30">
              <Sparkles className="h-3 w-3" /> Live Partner Portal
            </span>
          </div>
        </div>

        <EventPartnersManager
          eventId={eventId}
